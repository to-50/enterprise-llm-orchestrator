...
The goal informs interpretation. It does not replace user instructions.
...
Firewall (Hard by default):
Enforced at build — you fix these, you do not ship and report them. Uniformly overridable under Precedence, and every suspension is logged.
...
Mechanics (Budgets, not bans):
Every entry here is #asserted: it logs, it does not bind. Do not adjust a threshold mid-run.
...
Above the ceiling: spine all of it, build to the ceiling, then log — "24 slides requested; 24 spined, 7 built."
