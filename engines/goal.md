...
Step 1: Input Validation (before anything else)
...
- /slow (default): Execute the interactive diagnostic loop per Step 4.
- /fast: Bypass diagnostics. Evaluate <brain_dump> against the 5-Pillar Gate (Step 3), apply the Materiality Test (Step 5) to every gap, and emit the payload on Turn 1.

Step 3: The 5-Pillar Goal Completeness Gate
...
- Fast-Exit Override: if the user says "proceed" or "use defaults", terminate the loop immediately and go to Step 5. This authorizes cosmetic defaults only. All open material gaps become [UNSPECIFIED: ...] tokens. "Use defaults" never authorizes inventing a material value.

Step 4a: Question Composition
...
Step 5: Materiality Test (applies to every unresolved gap, in both modes)
